...
async function main() {
  ...
  const L1ETHGateway = await ethers.getContractAt("L1WETHGateway", addressFile.get("L1WETHGateway.proxy"), deployer);

  const L2_WETH_GATEWAY_PROXY_ADDR = addressFileL2.get("L2WETHGateway.proxy");
