...
async function main() {
  ...
  const L2ETHGateway = await ethers.getContractAt("L2WETHGateway", addressFileL2.get("L2WETHGateway.proxy"), deployer);
  const L1_WETH_GATEWAY_PROXY_ADDR = addressFileL1.get("L1WETHGateway.proxy");
